import React from 'react'
import ratingIcon from "../assets/rating.png"
import time_pic from "../assets/time_homepage.png"
import question from "../assets/questions_homepage.png"
const Card = () => {
    const [image, time, rating, title] = [
        "https://images.pexels.com/photos/4381392/pexels-photo-4381392.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
        3,
        4.5,
        "Course Title"
    ]
  return (
    //create a horizontal card with an image, title, and rating that is still horizontal on small screen
  
    <div className="flex items-center space-x-4 shadow-lg mx-4 rounded-md border-2 hover:border-sky-500 ">
      <img className="h-20 w-20 rounded ml-3 my-2" src={image} alt={title} />
      <div>
        <h3 className="text-lg font-bold font-ubuntu text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-cyan-500">{title}</h3>
              <div className='flex space-x-1'>
                <img className='mt-1' src={question} />
                <p className="text-gray-400">10 Questions</p>
              </div>
              <div className='flex space-x-2'>
                <img className='w-4 h-4 mt-1' src={time_pic} />
                <p className='text-gray-400'> {`${time} ${time > 1 ? "hours" : "hour"}`}</p>
              </div>
      </div>
    
      <div className='flex ml-auto mr-3 space-x-2'>
          <img className='w-4 h-4 mt-1 ' src={ratingIcon} />
          <p className=' font-medium text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-500'>{rating}</p>
      </div>
    </div>
  )}
export default Card